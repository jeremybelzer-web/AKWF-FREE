# Wish Myself Well

Live-action storyboard comic for a TV series about people wishing themselves well.

Open the comic:

```bash
cd "Wish Myself Well/web"
python3 -m http.server 8765
```

Then visit `http://localhost:8765`.

- **Watch** — Episodes 1–8 (through `Also Watching`). Every hour shows three other rooms on the same episode: a prompt addict, a lonely achiever, and one more.
- **Skits** — full bunk scripts; more voice memos welcome
- **People** — ensemble, including The Six, Ramanandji, Lark’s Lisbon listing, Nix/Vell (prompt flavors), Ayo/Gwen (grouped and suffering)
- **Songs** — The Six’s workshop: original hooks, verses, craft devices. No other artists’ lyrics.
- **The Lens** — 3D/4D chart of body, mind, feeling, relation, work, influence, love, seva; pain-body weather; witness as the unplotted center
- **World / Engines / Seeds / Bible** — the future that already happened, manifesto-as-scene, later episode cards, rules

Arrow keys (and space) move the episode.

Living notes: [`bible/SHOW-BIBLE.md`](bible/SHOW-BIBLE.md)
